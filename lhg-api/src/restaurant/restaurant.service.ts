...
export class RestaurantService {
  constructor(
    private pools: TenantPoolService,
    private kpiCacheService: KpiCacheService,
    private queryUtils: QueryUtilsService,
  ) {}

  async calculateKpisByDateRange(tenant: TenantConfig, startDate: Date, endDate: Date) {
    ...
    const currentResult = await this.kpiCacheService.getOrCalculate(
      ...
      async () => this._calculateKpisByDateRangeInternal(tenant, startDate, endDate),
    ...
    const previousResult = await this.kpiCacheService.getOrCalculate(
      ...
      CachePeriodEnum.CUSTOM,
      async () =>
        this._calculateKpisByDateRangeInternal(tenant, previousStartDate, previousEndDate),
    ...
    const monthlyResult = await this.kpiCacheService.getOrCalculate(
      ...
      'restaurant',
      CachePeriodEnum.CUSTOM,
      async () => this._calculateKpisByDateRangeInternal(tenant, monthStartDate, monthEndDate),
   ...
   */
  private async _calculateKpisByDateRangeInternal(
    tenant: TenantConfig,
    startDate: Date,
    endDate: Date,
  ) {
    // Classificação de tipos de produto (A&B) vem do tenant registry — varia por unidade.
    // "Outros" (Produtos Adicionais) é SEMPRE catch-all (NOT IN abProductTypes OR NULL),
    // garantindo que A&B + Outros = Receita de vendas. Ver reportByOthersSql / gráfico de grupos.
    const {
      abProductTypeIds: abProductTypes,
      aProductTypeIds: aProductTypes,
      bProductTypeIds: bProductTypes,
      aRankingIds: aProductTypesForRanking,
      bRankingIds: bProductTypesForRanking,
      aLeastRankingIds: aProductTypesForLeastRanking,
      bLeastRankingIds: bProductTypesForLeastRanking,
    } = tenant.restaurant;
...
`;
    ...
    try {
      const [
        ...
        this.pools.query<any>(tenant.slug, kpisRawSql),
        this.pools.query<any>(tenant.slug, revenueAbPeriodSql),
        this.pools.query<any>(tenant.slug, totalRevenueByPeriodSql),
        this.pools.query<any>(tenant.slug, abTicketCountByPeriodSql),
        this.pools.query<any>(tenant.slug, bestSellingFoodSql),
        this.pools.query<any>(tenant.slug, leastSellingFoodSql),
        this.pools.query<any>(tenant.slug, bestSellingDrinksSql),
        this.pools.query<any>(tenant.slug, leastSellingDrinksSql),
        this.pools.query<any>(tenant.slug, revenueGroupByPeriodSql),
        this.pools.query<any>(tenant.slug, revenueAPeriodSql),
        this.pools.query<any>(tenant.slug, revenueBPeriodSql),
        this.pools.query<any>(tenant.slug, reportByFoodSql),
        this.pools.query<any>(tenant.slug, reportByDrinkSql),
        this.pools.query<any>(tenant.slug, reportByOthersSql),
        this.pools.query<any>(tenant.slug, companyTotalRevenueSql),
      ...
      const revenueAbByPeriod = {
        ...
        series: dateKeys.map((key: any) => Number((abGrouped.get(key) || 0).toFixed(2))),
      };
      ...
      function buildRevenueSeries(rawData: any[], isMonthly: boolean) {
        ...
        const series = [...categoryMap.entries()].map(([category, dateMap]) => {
          const data = sortedDates.map((date: any) => Number((dateMap.get(date) || 0).toFixed(2)));
