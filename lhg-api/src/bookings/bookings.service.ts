...
export class BookingsService {
  ...
  private determineRentalPeriod(
    ...
    // Classificação por duração conforme os thresholds da unidade (registry)
    for (const threshold of tenant.rentalTypes) {
      if (occupationTimeSeconds <= threshold.maxSeconds) {
        return threshold.type;
      }
    }

    // Regras estendidas (DAY_USE/OVERNIGHT/DAILY) — só nas unidades que as usam
    if (tenant.extendedRentalRules && Booking && Array.isArray(Booking) && Booking.length > 0) {
      // Regra para Day Use
    ...
    // Fallback: o maior tipo por duração da unidade (padrão: TWELVE_HOURS)
    return tenant.rentalTypes[tenant.rentalTypes.length - 1].type;
  }
...
`;
    ...
    const [
      ...
      this.pools.query<any>(tenant.slug, totalBookingRevenueSQL),
      this.pools.query<any>(tenant.slug, totalBookingCountSQL),
      this.pools.query<any>(tenant.slug, totalRevenueSQL),
      this.pools.query<any>(tenant.slug, paymentMethodsSQL),
      this.pools.query<any>(tenant.slug, rentalTypeSQL),
      this.pools.query<any>(tenant.slug, billingPerChannelSQL),
      this.pools.query<any>(tenant.slug, billingByDateSQL),
      this.pools.query<any>(tenant.slug, ecommerceByDateSQL),
    ]);
