...
export class GovernanceService {
  ...
  async calculateKpibyDateRangeSQL(tenant: TenantConfig, startDate: Date, endDate: Date): Promise<any> {
   ...
   */
  private async _calculateKpibyDateRangeSQLInternal(
    tenant: TenantConfig,
    startDate: Date,
    endDate: Date,
  ): Promise<any> {
    // Fragmentos SQL específicos da unidade (tenant registry)
    const camareirasSqlList = tenant.governance.camareirasCargoIds.join(', ');
    const supervisorCargoId = tenant.governance.supervisorCargoId;
    const teamSizingCargoSqlList = tenant.governance.teamSizingCargoIds.join(', ');
    const terceirizadoWhenSql =
      tenant.governance.terceirizados.kind === 'byEmployeeIds'
        ? `WHEN f."id" IN (${tenant.governance.terceirizados.ids.join(', ')}) THEN 'Terceirizado'`
        : '';
    const excludedEmployeeSql = tenant.governance.excludedEmployeeIds.length
      ? `AND f."id" NOT IN (${tenant.governance.excludedEmployeeIds.join(', ')})`
      : '';
  ...
  WHERE l."datainicio" BETWEEN '${formattedStart}' AND '${formattedEnd}'
    ...
    AND f."id_cargo" IN (${camareirasSqlList})
`;
  ...
  WHERE va."datainicio" BETWEEN '${formattedStart}' AND '${formattedEnd}'
    ...
    AND f."id_cargo" = ${supervisorCargoId}
`;
...
`;
  ...
  WITH shift_data AS (
    ...
    WHERE
      ...
      AND l."motivofim" = 'COMPLETA'
      AND f."id_cargo" IN (${camareirasSqlList})
      AND f."id_cargo" NOT IN (${supervisorCargoId})
...
shifted_cleanings AS (
  ...
  WHERE l."datainicio" BETWEEN '${formattedStart}' AND '${formattedEnd}'
    AND l."datafim" IS NOT NULL
    AND l."motivofim" = 'COMPLETA'
    AND f."id_cargo" IN (${teamSizingCargoSqlList})
...
`;
    ...
    const [
      ...
      this.pools.query<{ totalSuitesCleaned: number }>(tenant.slug, totalSuitesCleanedSql),
      this.pools.query<{ totalInspections: number }>(tenant.slug, totalInspectionsSql),
      this.pools.query<{ name: string; value: number }>(tenant.slug, supervisorPerformanceSQL),
      this.pools.query<{ name: string; value: number }>(tenant.slug, shiftCleaningSQL),
      this.pools.query<any>(tenant.slug, cleaningsByPeriodSql),
      this.pools.query<any>(tenant.slug, cleaningsByPeriodShiftSql),
      this.pools.query<any>(tenant.slug, employeeReportSql),
      this.pools.query<TeamSizingRow>(tenant.slug, teamSizingSQL),
    ]);
    ...
    const cleaningsByPeriod = {
      ...
      series: dateKeys.map((key: any) => Number((cleaningsGrouped.get(key) || 0).toFixed(0))),
    };
