...
export class CompanyService {
  constructor(
    private readonly pools: TenantPoolService,
  ...
  // NOTE: fetchKpiData method removed during migration from Prisma to pg (node-postgres)
  // This method was unused and referenced the old PrismaService
  ...
  async calculateKpisByDateRangeSQL(
    ...
    // INDEPENDENTES entre si — rodam em PARALELO para cortar os round-trips
    // sequenciais (crítico em unidades de alta latência). Resultado IDÊNTICO:
    // são apenas combinadas depois; cada uma mantém seu próprio cache (chaves
    // distintas por range).
    const [currentResult, previousResult, monthlyResult] = await Promise.all([
      this.kpiCacheService.getOrCalculate(
        ...
        async () =>
          this._calculateKpisByDateRangeSQLInternal(tenant, startDate, endDate),
      ...
      this.kpiCacheService.getOrCalculate(
        ...
        CachePeriodEnum.CUSTOM,
        async () =>
          this._calculateKpisByDateRangeSQLInternal(
            tenant,
            previousStartDate,
            previousEndDate,
          ),
    ...
    if (daysElapsed > 0) {
      ...
      `;
      const totalSuitesResult: any[] = await this.pools.query<any>(tenant.slug, totalSuitesSQL);
  ...
  private async _calculateKpisByDateRangeSQLInternal(
    ...
    const bigNumbersSQL = `
      WITH receita_consumo AS (
        ...
        -- Filtrando apenas locações de apartamentos das categorias corretas (${suiteIdsSqlList})
        SELECT
        ...
        WHERE la.datainicialdaocupacao >= '${formattedStart}'
          ...
          AND ca_apt.id IN (${suiteIdsSqlList})
    ...
    const [
      ...
      this.pools.query<any>(tenant.slug, bigNumbersSQL),
      this.pools.query<any>(tenant.slug, totalSaleDirectSQL),
      this.pools.query<any>(tenant.slug, totalSuitesSQL),
      this.pools.query<any>(tenant.slug, revenueByDateSQL),
      this.pools.query<any>(tenant.slug, rentalRevenueByDateSQL),
      this.pools.query<any>(tenant.slug, billingRentalTypeSQL),
      this.pools.query<any>(tenant.slug, revenueBySuiteCategorySQL),
      this.pools.query<any>(tenant.slug, rentalsByDateSQL),
      this.pools.query<any>(tenant.slug, ticketAverageByDateSQL),
      this.pools.query<any>(tenant.slug, trevparByDateSQL),
      this.pools.query<any>(tenant.slug, occupancyRateBySuiteCategorySQL),
      this.pools.query<any>(tenant.slug, suitesByCategorySQL),
      this.pools.query<any>(tenant.slug, giroByDateSQL),
      this.pools.query<any>(tenant.slug, occupancyTimeByDateSQL),
    ]);
