...
async function bootstrap() {
  try {
    ...
    const servicePrefix = process.env.SERVICE_PREFIX_LIV || 'liv';
    app.setGlobalPrefix(`${servicePrefix}/api`);
    const isProduction = process.env.NODE_ENV === 'production';

    // Configuração do Swagger
    const swaggerConfig = new DocumentBuilder()
      .setTitle('LHG Analytics Backend - Liv')
    ...
    SwaggerModule.setup('liv/api', app, document, {
      swaggerOptions: {
      ...
      customSiteTitle: 'LHG Analytics - Liv',
    });
    console.log('Swagger UI disponível em: /liv/api');
    ...
    const port = process.env.PORT_LIV || 3006;
