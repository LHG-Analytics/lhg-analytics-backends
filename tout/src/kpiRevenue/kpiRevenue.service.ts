...
export class KpiRevenueService {
  ...
  async findAllKpiRevenue(startDate: Date, endDate: Date, period?: PeriodEnum): Promise<any> {
    try {
      ...
      throw new BadRequestException(`Failed to fetch KPI Revenue data: ${errorMessage}`);
    }
  }
  ...
  private async accumulateByRentalType(
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      throw new BadRequestException(`Failed to accumulate data by rental type: ${errorMessage}`);
    }
  }
  ...
  private async calculateTotalRevenueByPeriod(
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      throw new BadRequestException(`Failed to calculate total revenue by period: ${errorMessage}`);
    }
  }
