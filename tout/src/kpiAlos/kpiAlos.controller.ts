...
import { ApiTags, ApiQuery, ApiNotFoundResponse, ApiBadRequestResponse } from '@nestjs/swagger';
import { PeriodEnum } from '@client-online';

@ApiTags('KpiAlos')
@Controller('KpiAlos')
export class KpiAlosController {
  constructor(private readonly kpiAlosService: KpiAlosService) {}

  @Get('create-and-find-all-kpi-alos')
  @ApiQuery({
    name: 'startDate',
    required: true,
  ...
  @ApiNotFoundResponse({ description: 'No KPI Alos found.' })
  @ApiBadRequestResponse({ description: 'Failed to fetch KPI Alos.' })
  async createAllKpiAlos(
    ...
    try {
      ...
      throw new BadRequestException(`Failed to create all KpiAlos: ${error.message}`);
    }
  }
