...
export class CleaningsService {
  ...
  async findAllCleanings(startDate: Date, endDate: Date, period?: PeriodEnum): Promise<any> {
    try {
      ...
      throw new BadRequestException(`Failed to fetch Cleanings: ${errorMessage}`);
    }
  }
  ...
  private async calculateCleaningsByWeek(
    ...
    try {
      ...
      for (const shift in cleaningsByShiftAndDay) {
        ...
        cleaningsByShiftAndDay[shift].realShiftMaid = uniqueEmployeesByShift[shift].size;
  ...
  private async calculateCleaningsByShift(
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      throw new BadRequestException(`Failed to calculate cleanings by shift: ${errorMessage}`);
    }
  }
