...
@ApiTags('KpiRevpar')
@Controller('KpiRevpar')
export class KpiRevparController {
  constructor(private readonly kpiRevparService: KpiRevparService) {}

  @Get('create-and-find-all-kpi-revpar')
  @ApiQuery({
    name: 'startDate',
    required: true,
  ...
  @ApiNotFoundResponse({ description: 'No KPI Revpar found.' })
  @ApiBadRequestResponse({ description: 'Failed to fetch KPI Revpar.' })
  async createAllKpiRevpar(
    ...
    try {
      ...
      return await this.kpiRevparService.findAllKpiRevpar(start, end, period);
    } catch (error) {
      throw new BadRequestException(
        `Failed to create all KpiRevpar: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
    }
  }
  ...
  async runCronJob(): Promise<any> {
    try {
      ...
      return await this.kpiRevparService.handleCron();
    } catch (error) {
      throw new BadRequestException(
