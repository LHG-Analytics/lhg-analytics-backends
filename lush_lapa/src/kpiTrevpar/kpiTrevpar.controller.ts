...
@ApiTags('KpiTrevpar')
@Controller('KpiTrevpar')
export class KpiTrevparController {
  constructor(private readonly kpiTrevparService: KpiTrevparService) {}

  @Get('create-and-find-all-kpi-trevpar')
  @ApiQuery({
    name: 'startDate',
    required: true,
  ...
  @ApiNotFoundResponse({ description: 'No KPI Trevpar found.' })
  @ApiBadRequestResponse({ description: 'Failed to fetch KPI Trevpar.' })
  async createAllKpiTrevpar(
    ...
    try {
      ...
      return await this.kpiTrevparService.findAllKpiTrevpar(start, end, period);
    } catch (error) {
      throw new BadRequestException(
        `Failed to create all KpiTrevpar: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
    }
  }
  ...
  async runCronJob(): Promise<any> {
    try {
      ...
      return await this.kpiTrevparService.handleCron();
    } catch (error) {
      throw new BadRequestException(
