...
export class RestaurantSalesController {
  ...
  async createAndFindAllRestaurantSales(
    ...
    try {
      ...
      return await this.restaurantSalesService.findAllRestaurantSales(start, end, period);
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      throw new BadRequestException(`Failed to create all RestaurantSales: ${errorMessage}`);
    }
  }
