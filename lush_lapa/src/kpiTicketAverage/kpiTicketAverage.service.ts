...
export class KpiTicketAverageService {
  ...
  async findAllKpiTicketAverage(startDate: Date, endDate: Date, period?: PeriodEnum): Promise<any> {
    try {
      ...
      const formattedKpiTicketAverageData = kpiTicketAverageData.map((category: any) => ({
        ...category,
  ...
  private async calculateTotalTicketAverageByPeriod(
    ...
    try {
      ...
      const totalTicketAverageForThePeriod = Object.keys(results).map((date: any) => ({
        [date]: results[date],
      }));
  ...
  async handleCron() {
    ...
    await this.findAllKpiTicketAverage(parsedStartDateEsteMes, parsedEndDateEsteMes, PeriodEnum.ESTE_MES);
