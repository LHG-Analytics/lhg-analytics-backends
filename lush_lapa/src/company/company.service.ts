  ...
  private async _calculateKpisByDateRangeSQLInternal(
    ...
    const revenueBySuiteCategorySQL = `
      ...
      WHERE la.datainicialdaocupacao >= '${formattedStart}'
        AND la.datainicialdaocupacao <= '${formattedEnd}'
        AND la.fimocupacaotipo = 'FINALIZADA'
        AND ca.descricao IN ('LUSH', 'LUSH HIDRO', 'LUSH LOUNGE HIDRO', 'LUSH SPA', 'LUSH SPLASH', 'LUSH SPA SPLASH', 'LUSH SUBLIME', 'LUSH SPA SUBLIME')
    ...
    const suitesByCategorySQL = `
      ...
      FROM apartamento a
      INNER JOIN categoriaapartamento ca ON a.id_categoriaapartamento = ca.id
      WHERE ca.descricao IN ('LUSH', 'LUSH HIDRO', 'LUSH LOUNGE HIDRO', 'LUSH SPA', 'LUSH SPLASH', 'LUSH SPA SPLASH', 'LUSH SUBLIME', 'LUSH SPA SUBLIME')
        AND a.dataexclusao IS NULL
      GROUP BY ca.descricao
    `;
    ...
    if (bigNumbersResult && bigNumbersResult.length > 0) {
      ...
      try {
        ...
        `;
        const investigationResult = await this.pgPool.query<any>(investigationQuery);
    ...
    `;
    const suiteCategoryKpisResult: any[] = await this.pgPool.query<any>(suiteCategoryKpisQuery);
    ...
    `;
    const giroByWeekResult: any[] = await this.pgPool.query<any>(giroByWeekQuery);
    ...
    `;
    const revparByWeekResult: any[] = await this.pgPool.query<any>(revparByWeekQuery);
