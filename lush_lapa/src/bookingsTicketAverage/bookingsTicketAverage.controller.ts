...
export class BookingsTicketAverageController {
  ...
  @ApiBadRequestResponse({ description: 'Failed to calculate ticket average.' })
  @ApiQuery({
    name: 'startDate',
    required: true,
  ...
  @ApiQuery({
    name: 'period',
    required: false,
    description: 'Período para o qual a média de tickets será calculada',
    example: 'LAST_7_D',
    enum: PeriodEnum,
  })
  async createAndFindAllBookingsTicketAverage(
    ...
    try {
      ...
      return await this.bookingsTicketAverageService.findAllBookingsTicketAverage(
        start,
        end,
        period,
      );
