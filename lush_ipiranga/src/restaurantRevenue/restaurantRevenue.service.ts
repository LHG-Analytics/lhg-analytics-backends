  ...
  async findAllRestaurantRevenue(
    ...
    try {
      ...
      for (const rentalApartment of allRentalApartments) {
        ...
        if (saleLease && saleLease.stockOutId) {
          ...
          if (stockOutSaleLease && Array.isArray(stockOutSaleLease.stockOutItem)) {
            ...
            stockOutSaleLease.stockOutItem.forEach(
              (stockOutItem: { priceSale?: string | number; quantity?: string | number }) => {
      ...
      if (error instanceof Error) {
        ...
        console.error('Erro ao buscar Restaurant Revenue data:', error);
        throw new BadRequestException('Failed to fetch Restaurant Revenue data: erro desconhecido');
      }
    }
  }
