...
export class KpiAlosService {
  ...
  async handleCron() {
    ...
    console.log(`Início CronJob KpiAlos - este mês: ${startTimeEsteMes}`);

    // Chamar apenas a função findAllKpiAlos para o período ESTE_MES
    await this.findAllKpiAlos(parsedStartDateEsteMes, parsedEndDateEsteMes, PeriodEnum.ESTE_MES);
    const endTimeEsteMes = moment().tz(timezone).format('DD-MM-YYYY HH:mm:ss');
    console.log(`Final CronJob KpiAlos - este mês: ${endTimeEsteMes}`);
  }
