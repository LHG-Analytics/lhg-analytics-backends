...
@ApiTags('KpiTicketAverage')
@Controller('KpiTicketAverage')
export class KpiTicketAverageController {
  constructor(private readonly kpiTicketAverageService: KpiTicketAverageService) {}

  @Get('create-and-find-all-kpi-ticket-average')
  @ApiQuery({
    name: 'startDate',
    required: true,
  ...
  @ApiBadRequestResponse({
    description: 'Failed to fetch KPI Ticket Averages.',
  })
  async createAllKpiTicketAverage(
    ...
    try {
      ...
      return await this.kpiTicketAverageService.findAllKpiTicketAverage(start, end, period);
    } catch (error) {
      throw new BadRequestException(`Failed to create all KpiTicketAverage: ${error.message}`);
    }
  }
