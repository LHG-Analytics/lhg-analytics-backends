...
export class RestaurantRevenueController {
  ...
  @ApiBadRequestResponse({
    description: 'Failed to create all restaurant revenue.',
  })
  @ApiQuery({
    name: 'startDate',
    required: true,
  ...
  async createAndFindAllRestaurantRevenue(
    ...
    try {
      ...
      throw new BadRequestException(`Failed to create all RestaurantRevenue: ${error.message}`);
    }
  }
  ...
  @Get('run-cron-job')
  @HttpCode(HttpStatus.OK)
  @ApiNotFoundResponse({ description: 'No Restaurant Revenue found.' })
