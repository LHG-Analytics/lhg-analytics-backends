...
export class KpiOccupancyRateService {
  ...
  async findAllKpiOccupancyRate(
    ...
    try {
      ...
      const categoryTotalsMap = {}; // Mapa para acumular os dados por categoria
      ...
      throw new BadRequestException(`Failed to fetch KpiOccupancyRate: ${error.message}`);
    }
  }
  ...
  private async calculateTotalOccupancyRateByPeriod(
    ...
    try {
      ...
      throw new BadRequestException(
        `Falha ao calcular total de Occupancy Rate por período: ${error.message}`,
      );
    }
  }
  ...
  async handleCron() {
    ...
    await this.findAllKpiOccupancyRate(parsedStartDateEsteMes, parsedEndDateEsteMes, PeriodEnum.ESTE_MES);
